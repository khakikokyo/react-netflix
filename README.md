...
`Nav.js` scroll 이벤트를 적용한 네비게이션 바를 생성하고, BrowserRouter를 사용하여 3개의 pages를 구분하여 `App.js` 메인 화면을 구현하였습니다. axios 외부 라이브러리로 `The Movie DB API` 현재 상용중인 영화 정보, 아이디, 특정 영화의 상세 정보를 가져와 메인 이미지 배너를 생성하고, play 버튼을 클릭시 비디오로 전환, Iframe으로 유튜브 비디오를 재생합니다.
